...
def get_scatter_plot(entered_site, payload_value):
    scatter_df = spacex_df[spacex_df['Payload Mass (kg)'].between(payload_value[0],payload_value[1])]
    if entered_site == 'ALL':
        ...
        fig = px.scatter(scatter_df, x ='Payload Mass (kg)', y='class', color='Booster Version Category',title =f'Correlation between Payload and Success for {entered_site}')
        return fig
